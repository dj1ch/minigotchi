...
void Parasite::sendName() {
    if (Config::parasite) {
        if (Config::name.length() > 25) {
            // Pwnagotchi names can be 25 characters max, so want to limit our Minigotchi's name to that as well
            // Will truncate it to 22 characters + "..." if it exceeds 25
            char buf[26];
...
void Parasite::sendDeauthStatus(parasite_deauth_status_type_t status,
    ...
    if (Config::parasite) {
        if (target != nullptr && channel > 0) {
            ...
            // Unlikely scenario but will truncate to 29 characters + "..." in case that gets disrespected by someone
            if (strlen(target) > 32) {
                char targetBuf[33];
