...
bool Deauth::select() {
    ...
    if (apCount > 0 && Deauth::randomIndex == -1) {
        ...
        if (encType == -1 || encType == ENC_TYPE_NONE) {
            ...
            Display::updateDisplay("('-')", "Selected AP is not encrypted. Skipping deauthentication...");
        ...
        if (std::find(whitelist.begin(), whitelist.end(), randomAP) !=
            ...
            Serial.println("('-') Selected AP is in the whitelist. Skipping "
                           "deauthentication...");
            Display::updateDisplay("('-')", "Selected AP is in the whitelist. Skipping deauthentication...");
        ...
        Display::updateDisplay("('-')", "Full AP SSID: " +
                               WiFi.SSID(Deauth::randomIndex));

        Serial.print("('-') AP Encryption: ");
        Serial.println(WiFi.encryptionType(Deauth::randomIndex));
        Display::updateDisplay("('-')", "AP Encryption: " +
